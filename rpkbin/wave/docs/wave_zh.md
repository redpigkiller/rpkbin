...
- **安裝：** `python -m pip install -e ".[wave]"`；這會提供 `rpk-wave` CLI，以及 Textual、prompt-toolkit、Rich 支援。
- **Public entry points：** `session`、`CmdJob`、`FuncJob`、`PtyJob`、`PtyCmdJob`、`RegexParser`、`StatefulParser`、`Hook` 與 `rpk-wave`。
- **Runtime boundary：** Wave 載入 trusted local Python wave file，並透過本機 `JobManager` 執行 jobs；它不是 remote scheduler。
- **Modes：** interactive terminal 預設使用 TUI；CI 與 pipe 應使用 `--no-tui`。精確 options 與 exit behavior 請看 [CLI reference](../../../docs/cli-reference_zh.md)。
- **狀態：** Stable。**驗證：** `python -m pytest tests/wave -q`。
- **相關文件：** [English guide](wave.md)、[package 架構](../../../docs/architecture_zh.md) 與 [Job Manager](../../../docs/job_manager/job_manager_zh.md)。
