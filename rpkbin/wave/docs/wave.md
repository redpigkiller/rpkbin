...
- **Install:** `python -m pip install -e ".[wave]"`; this provides the `rpk-wave` CLI plus Textual, prompt-toolkit, and Rich support.
- **Public entry points:** `session`, `CmdJob`, `FuncJob`, `PtyJob`, `PtyCmdJob`, `RegexParser`, `StatefulParser`, `Hook`, and `rpk-wave`.
- **Runtime boundary:** Wave loads a trusted local Python wave file and executes jobs through the local `JobManager`; it is not a remote scheduler.
- **Modes:** interactive terminals use the TUI by default; CI and pipes should use `--no-tui`. Use the [CLI reference](../../../docs/cli-reference.md) for exact options and exit behavior.
- **Status:** Stable. **Verification:** `python -m pytest tests/wave -q`.
- **Related:** [Traditional Chinese guide](wave_zh.md), [package architecture](../../../docs/architecture.md), and [Job Manager](../../../docs/job_manager/job_manager.md).
