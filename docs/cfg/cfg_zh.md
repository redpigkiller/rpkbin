...
每個 `(src, dst)` 僅能有一條 structural edge。重複呼叫 `add_edge()` 會
raise `ValueError`，且不會覆寫既有 attributes；請以
`update_edge(src, dst, **attrs)` 做明確的 partial update（包括
`cond=None`）。所有輸入 attributes，以及 `edges`、`edge_attrs()`、
`out_edges()`、`in_edges()`、`remove_edge()` 的輸出，皆為包含 nested
values 的 deep copy。

`entry_id` 與 `exit_id` 是唯讀的 configured ids。`terminal_blocks()`
以 insertion order 回傳 entry-reachable terminals；
`terminal_blocks(reachable_only=False)` 回傳所有 terminals。核心
`remove_unreachable()` 會依 insertion order 移除並回傳 unreachable
blocks。`to_networkx()` 回傳獨立的 deep `networkx.DiGraph` snapshot，
不是 live editing handle。
...
# callee 會排在 caller 前；recursive SCC 內使用 deterministic heuristic
program.function_order("bottom_up")

# 依函數名稱的 lexicographic 順序，適合 deterministic generated artifacts
...
- `"cond_aware_weight"`：先偏好 unconditional edge，再以 weight 排同一
  conditionality 的 successor。
...
cfg.post_dominators()  # 使用 configured exit；也可傳 exit_node="done"
cfg.dominator_tree()
```

Dominance 只定義於從 selected `start`（或 configured entry）可達的 blocks。
`dominates()` 對 unknown block 或 start 會 raise `KeyError`；若既有的
dominator 或 node 從 selected start 不可達，會 raise `ValueError`。
`dominator_tree()` 也只包含 reachable blocks；unreachable blocks 不會成為
isolated tree nodes。

`post_dominators(exit_node=None)` 是 single-selected-exit post-dominance：
explicit `exit_node` 優先，否則使用 configured exit。mapping 只包含能到達
selected exit 的 blocks，且 selected exit 映射到自己。此 API 不會為
multi-terminal CFG 建立 virtual exit，也不是 multi-exit control-dependence
analysis。若需要分析特定 terminal，先用 `terminal_blocks()` 檢查，再明確
選擇 exit。若沒有 explicit exit，也沒有 configured exit，會 raise
`RuntimeError`；請呼叫 `set_exit()` 或傳入 `exit_node`。
...
| `CallRef(callee, raw="")` | 標註對 `Program` 中另一個 CFG 的呼叫；callee may-def 會向外傳遞，但只有 definite write 會消滅 caller liveness。 |
...
`BasicBlock` 以 id 比較，但刻意不可 hash：`CFG.rename_block()` 可以合法改變
owned block 的 id，因此不能把 block 放入 set 或當作 dict key。空的
`OtherInsn.defs` / `.uses` 只表示 caller 未提供已知 dataflow effect；若 instruction
實際讀寫變數，liveness 會 under-approximate。`FunctionSummary.defs` 是 entry-reachable
may-def，`must_defs` 只包含每條 reachable normal-return path 都寫入的值；沒有 normal
return 時是空集合。

### Correctness 與 ownership contract

`BasicBlock.id` 是非空、不可變的 graph-node identity；已加入 CFG 的 block 請使用
`CFG.rename_block()` 更名。`CFG.add_block(BasicBlock(...))`、`CFG.copy()` 與
`merge_cfgs()` 會獨立擁有 instructions、metadata 與 edge attributes。

`CFG.validate()` 會檢查 node key/id 和 instruction type；建構後才加入的未知 `CallRef`
由 `Program.validate()` 回報。`FunctionSummary.defs` 保持 may-def，只有指定 exit 的
所有路徑都會寫入的 `must_defs` 會在 call site 消滅 caller liveness。

`diff_cfgs()` 會報告 entry/exit 變更；`align_by="label"` 時 logical label 相同即不算
變更。MCU weight policy 要求有限、非 `bool`、且 >= 0 的數值。`CFG.export_dot()` 請
安裝 `rpkbin[dot]`。
